...
struct RISCV_Instruction {
    ...
    void get_S_opName(const int funct3) {
        if (funct3 == 0) this->opName = "sb";
        else if (funct3 == 1) this->opName = "sh";
        else if (funct3 == 2) this->opName = "sw";
    }

    void get_R_opName(const int funct7, const int funct3) {
        if (funct3 == 0 && funct7 == 0) this->opName = "add";
        else if (funct3 == 0 && funct7 == 32) this->opName = "sub";
        else if (funct3 == 7) this->opName = "and";
        else if (funct3 == 6) this->opName = "or";
        else if (funct3 == 4) this->opName = "xor";
        else if (funct3 == 1) this->opName = "sll";
        else if (funct3 == 2) this->opName = "slt";
        else if (funct3 == 3) this->opName = "sltu";
        else if (funct3 == 5 && funct7 == 0) this->opName = "srl";
        else if (funct3 == 5 && funct7 == 32) this->opName = "sra";
    }

    void get_I_opName(const int funct3, const int opcode_decimal, const int funct7) {
        if (funct3 == 5 && funct7 == 0) this->opName = "srli";
        else if (funct3 == 5 && funct7 == 32) this->opName = "srai";
        else if (opcode_decimal == 103) this->opName = "jalr";
        else if (funct3 == 0 && opcode_decimal == 3) this->opName = "lb";
        else if (funct3 == 1 && opcode_decimal == 3) this->opName = "lh";
        else if (funct3 == 2 && opcode_decimal == 3) this->opName = "lw";
        else if (funct3 == 0) this->opName = "addi";
        else if (funct3 == 7) this->opName = "andi";
        else if (funct3 == 6) this->opName = "ori";
        else if (funct3 == 4) this->opName = "xori";
        else if (funct3 == 2) this->opName = "slti";
        else if (funct3 == 3) this->opName = "sltiu";
        else if (funct3 == 1) this->opName = "slli";
    }

    void get_SB_opName(const int f3) {
        if (f3 == 0) this->opName = "beq";
        else if (f3 == 1) this->opName = "bne";
        else if (f3 == 4) this->opName = "blt";
        else if (f3 == 5) this->opName = "bge";
        else if (f3 == 6) this->opName = "bltu";
        else if (f3 == 7) this->opName = "bgeu";
    }
...
int get_alu_ctrl(int ALUOp, int funct3, int funct7, int /*opcode*/) {
    if (ALUOp == 0) return 0x2;
    if (ALUOp == 1) return 0x6;
    if (ALUOp == 2) {
        if (funct3 == 0 && funct7 == 0) return 0x2;
        if (funct3 == 0 && funct7 == 32) return 0x6;
        if (funct3 == 7) return 0x0;
        if (funct3 == 6) return 0x1;
        if (funct3 == 0) return 0x2;
        if (funct3 == 7) return 0x0;
        if (funct3 == 6) return 0x1;
    }
    return 0x2;
...
void ControlUnit(int opcode, int /*funct3*/, int /*funct7*/, ControlSignals& c) {
    c = ControlSignals{};
    if (opcode == 51) {
        c.RegWrite = 1; c.ALUSrc = 0; c.MemtoReg = 0; c.ALUOp = 2;
        return;
    }
    if (opcode == 19) {
        c.RegWrite = 1; c.ALUSrc = 1; c.MemtoReg = 0; c.ALUOp = 2;
        return;
    }
    if (opcode == 3) {
        c.RegWrite = 1; c.MemRead = 1; c.ALUSrc = 1; c.MemtoReg = 1; c.ALUOp = 0;
        return;
    }
    if (opcode == 35) {
        c.MemWrite = 1; c.ALUSrc = 1; c.ALUOp = 0;
        return;
    }
    if (opcode == 99) {
        c.Branch = 1; c.ALUSrc = 0; c.ALUOp = 1;
        return;
    }
    if (opcode == 111) {
        c.RegWrite = 1; c.Jump = 1; c.ALUOp = 0;
        return;
    }
    if (opcode == 103) {
        c.RegWrite = 1; c.Jalr = 1; c.ALUSrc = 1; c.MemtoReg = 0; c.ALUOp = 0;
        return;
    }
}

static bool insn_uses_rs1(const RISCV_Instruction& i) {
    if (i.type == "UJ" || i.type == "U") return false;
    return true;
}
...
void ex_compute(const ID_EX& in, int32_t& alu_result, int& alu_zero, int32_t& branch_target) {
    ...
    else if (instr.opName.size() >= 2 && instr.opName.substr(0, 2) == "be") opcode = 99;
    else if (instr.opName == "beq") opcode = 99;
    else if (instr.opName == "jal") opcode = 111;
    else if (instr.opName == "jalr") opcode = 103;
    else if (instr.type == "SB") opcode = 99;

    int alu_ctrl = get_alu_ctrl(in.ctrl.ALUOp, instr.funct3, instr.funct7, opcode);
    int32_t op2 = in.ctrl.ALUSrc ? in.imm : in.rs2_val;
    switch (alu_ctrl) {
        case 0x0: alu_result = in.rs1_val & op2; break;
        case 0x1: alu_result = in.rs1_val | op2; break;
        case 0x2: alu_result = in.rs1_val + op2; break;
        case 0x6: alu_result = in.rs1_val - op2; break;
        default:  alu_result = in.rs1_val + op2; break;
    }
    alu_zero = (alu_result == 0) ? 1 : 0;
    branch_target = in.npc + (in.imm << 1);
    if (in.ctrl.Jump)
...
bool branch_taken_from_ex(const ID_EX& in, int alu_zero) {
    ...
    if (in.ctrl.Jump || in.ctrl.Jalr) return true;
    if (in.ctrl.Branch && alu_zero) return true;
    return false;
}
...
bool data_hazard_stall(const ID_EX& ex, const EX_MEM& mem, const MEM_WB& wb, const IF_ID& fd) {
    ...
    auto conflicts = [&](int r) -> bool {
        if (r == 0) return false;
        if (ex.valid && ex.ctrl.RegWrite && ex.rd_num == r) return true;
        if (mem.valid && mem.ctrl.RegWrite && mem.rd_num == r) return true;
        if (wb.valid && wb.ctrl.RegWrite && wb.rd_num == r) return true;
        return false;
    };
    if (insn_uses_rs1(d) && conflicts(d.rs1_num)) return true;
    if (insn_uses_rs2(d) && conflicts(d.rs2_num)) return true;
    return false;
}
...
static string fetch_instr(const vector<string>& program, int32_t fetch_pc) {
    if (fetch_pc < 0) return "";
    size_t idx = (size_t)(fetch_pc / 4);
    if (idx >= program.size()) return "";
    return program[idx];
}
...
void print_cycle_trace(int prev_rf[32], int prev_d_mem[32],
    ...
    if (wb_snapshot.valid && wb_snapshot.ctrl.RegWrite && wb_snapshot.rd_num > 0) {
        if (rf[wb_snapshot.rd_num] != prev_rf[wb_snapshot.rd_num]) {
            if (use_abi_names) cout << reg_name(wb_snapshot.rd_num);
            else cout << "x" << wb_snapshot.rd_num;
